...
# HARDENED: disable any .netrc usage globally and avoid env-based auth/proxy lookups
...
# ---- HTTP client (hardened against .netrc) ----------------------------------
import requests
import requests.utils as _rq_utils

# Global kill switch: prevent requests from ever consulting .netrc
_rq_utils.get_netrc_auth = lambda *a, **k: None  # type: ignore
...
_HTTP.auth = _NoOpAuth()         # prevents .netrc fallback even if auth unset
_HTTP.trust_env = False          # ignore env proxies/CA that could surprise users
# Optionally set a UA to keep behavior consistent
_HTTP.headers.update({"User-Agent": "EzBrowser/1.0"})
...
class JacintoLobbyBrowser(QWidget):
    ...
    def _refresh_available(self):
        try:
            res = _HTTP.get(f"{BACKEND_URL}/servers", timeout=3)
    ...
    def _hb_loop(self, host: Host, stop_event: threading.Event):
        ...
        while not stop_event.is_set():
            try:
                ...
                r = _HTTP.post(f"{BACKEND_URL}/add_server", json=payload, timeout=3)
